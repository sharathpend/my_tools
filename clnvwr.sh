...
clnvwr(){	
	...
	#===========================================================================
	TOP=$(pwd| cut -d'/' -f 1-6)
	SORTED_VWRS=$(find . -name "*.vwr*" -exec ls -alS {} \;|sort -r -t' ' -nk5)
	DATE=$(echo "$SORTED_VWRS"|awk '{print $6" "$7}')
	export SIZE=$(echo "$SORTED_VWRS"|awk '{print $5}')
	SIZE_WITH_UNITS=""
	for num in $SIZE; do
	...
	PATHS=$(echo "$SORTED_VWRS"|awk '{print $9}')
	LEN=$(echo "$PATHS"|awk '{print length" "$1}'|sort -r -t' ' -nk1|awk '{print $1}'|head -n 1)
