...
clnvcd(){	
	...
	#===========================================================================
	TOP=$(pwd| cut -d'/' -f 1-6)
	SORTED_VCDS=$(find . -name "*.vcd*" -exec ls -alS {} \;|sort -r -t' ' -nk5)
	DATE=$(echo "$SORTED_VCDS"|awk '{print $6" "$7}')
	export SIZE=$(echo "$SORTED_VCDS"|awk '{print $5}')
	SIZE_WITH_UNITS=""
	for num in $SIZE; do
	...
	PATHS=$(echo "$SORTED_VCDS"|awk '{print $9}')
